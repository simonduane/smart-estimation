...
Half-hourly usage data are associated with a large number of intervals, about 17,500 per year, all half an hour long. It would be extravagant to label every interval by two times, and so it is very common to use either the start time or the end time and not both. The source of confusion arises because data may be saved with the interval end time, but used later as if it were the interval start time. The error is easy enough to catch when looked for but, if not checked, will remain there indefinitely. That is precisely the situation Ovo finds itself in, whether or not it is aware of the error, the error is there, still, despite being told about it. I understand their data well enough that I can safely use the data and correct the error, and the scripts in this repository do this. I can even guess how the confusion arises: the moment when an interval ends is the moment that the usage *comes into existence*. Until that moment the value, how much would be used, was not yet known. Once the moment at the end of the interval has passed the value is known and the usage can be recorded. It is fairly natural to see that, in time-stamping the value, it becomes labelled with the interval end time. This may occur to the programmer writing code to process the usage data later, or it may not. In Ovo's case, apparently it did not. Whatever the reason might be, though, Ovo's half-hourly usage dataset is (somewhat ironically) labelled with both start time and end time, but those times are all wrong by 30 minutes.

### Model-based estimation

Estimates are best made using a model which can predict usage. The model must satisfy basic requirements, for instance that predicted usage must never exceed the supply rating (typically 25 kW for electricity, 6 m^3/hour for gas), and similarly must never be negative. But that's it, really, because the estimation process outlined here includes a *calibration* of the model[^(1)]. This calibration is what *can* make sure that model, i.e. all its predictions, is consistent with the customer's actual usage. In fact, the estimates can be surprisingly robust.
