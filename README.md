...
This account is set out like a mathematical proof by induction: I start by supposing that the algorithm, i.e. the processing, has worked so far, up to yesterday ("Suppose that the assertion is true for N."). The aim is to show that it can work up to today as well (this is the "inductive step"). The "snapshot" is the just the result of applying the algorithm.

There is a history of daily meter readings, one for each day up to and including yesterday. Each one is the "opening" reading for the day, preferably one retrieved from the smart meter. That preference is expressed by giving the reading a "status" which is highest if it is an actual reading (rather than an estimate). If that retrieval failed for whatever reason, then the reading status, whatever it might be, is lower. It turns out to be sufficient to allow for 4 different levels for the status of a meter reading: actual, inferred, estimated, or provisional. But the reading is never missing altogether. Data are not allowed to go missing, we just have to accept that some data are more reliable than others: they have higher status.
The reading value may in fact be the result of applying some correction to a provisional value, but it also turns out that we don't need to keep a full audit trail of such corrections - it is enough to allow each reading to have its status, one of the 4 possibilities listed. Only provisional readings may be subject to correction. To repeat, the status of a reading in the history of all readings is to be one of actual, inferred, estimated or provisional. Most actual readings arrive as values retrieved from the smart meter, but they could be manual readings by the customer or by someone else. I shall pretend that the the only actual readings are smart readings, and so I'll call them smart rather than actual.

There is also a complete history of usages, which I shall assume have half-hour resolution. Each one is the consumption during a particular 30 minute interval. These usages also have a status, which is smart, inferred, estimated or provisional. 
